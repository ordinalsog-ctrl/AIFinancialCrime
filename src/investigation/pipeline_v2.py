...
from src.investigation.serial_actor import SerialActorEngine
...
class InvestigationResult:
    ...
    serial_matches: list = field(default_factory=list)  # SerialMatch list
    ...
    def summary(self) -> dict:
        return {
            ...
            "serial_cases":         list({c for m in self.serial_matches
                                          for c in m.matching_cases}),
...
class InvestigationPipeline:
    ...
    def __init__(
        ...
        self._cio        = cio_engine
        self._serial     = serial_engine   # None = Serientäter-Check deaktiviert
    ...
    def run(
        ...
        victim_name: str = "",
        victim_contact: str = "",
        known_chains: Optional[list] = None,   # für Offline-Serientäter-Check
    ) -> InvestigationResult:
        """
        Full pipeline:
        ...
        # ── Step 7: Serientäter-Erkennung ──────────────────────────────
        serial_matches = []
        if self._serial is not None:
            # Online-Modus: gegen DB querieren
            serial_matches = self._serial.check_new_investigation(chain, 0)
        elif known_chains:
            # Offline-Modus: gegen mitgegebene Chains prüfen
            engine = SerialActorEngine(conn=None)
            serial_matches = engine.check_offline(chain, known_chains)

        if serial_matches:
            logger.warning(
                f"[{case_id}] ⚠ SERIENTÄTER: {len(serial_matches)} Treffer in "
                f"{list({c for m in serial_matches for c in m.matching_cases})}"
            )
