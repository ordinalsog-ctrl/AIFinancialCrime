...
from src.investigation.exchange_contacts import ExchangeContactDB as _ExchangeContactDB

_exchange_db = _ExchangeContactDB()
...
def _section_cover(chain: InvestigationChain, styles: dict, generated_at: str,
                   victim_name: str = "", victim_contact: str = "",
                   language: str = "de") -> list:
    story = []
    ...
    # Opfer-Daten einfügen wenn vorhanden
    if victim_name or victim_contact:
        meta_rows.insert(0, ["Erstattet durch", victim_name or "—"])
        if victim_contact:
            meta_rows.insert(1, ["Kontakt Opfer",   victim_contact])
...
def _section_recommended_actions(chain: InvestigationChain, styles: dict,
                                  language: str = "de") -> list:
    story = [PageBreak(), Paragraph("4. Empfohlene Massnahmen", styles["h1"])]
    story += _hr(styles)

    exchange_hits = chain.exchange_hits
    exchange_names = sorted(set(h.exchange_name for h in exchange_hits))
    has_exchange   = bool(exchange_hits)
    has_sanctioned = any(getattr(h, "is_sanctioned", False) for h in chain.hops)
    has_l1         = any(h.confidence.name == "L1_VERIFIED_FACT" for h in chain.hops)
    amount_btc     = chain.fraud_amount_btc or 0

    # ── Schritt 1: Sofortmassnahme ──────────────────────────────────────────
    urgency_text = (
        "<b>SOFORT handeln.</b> Kryptowerte können innerhalb von Minuten "
        "weiterbewegt werden. Je früher ein Freeze-Request die Exchange erreicht, "
        "desto höher die Erfolgschance. Die meisten Exchanges reagieren auf "
        "Law-Enforcement-Anfragen innerhalb von 24–72 Stunden wenn eine "
        "offizielle Strafanzeige vorliegt."
        if has_exchange else
        "<b>Zeitkritisch:</b> Obwohl in dieser Analyse keine Exchange direkt "
        "identifiziert wurde, können die Gelder jederzeit weiterbewegt werden. "
        "Einreichen der Strafanzeige und manuelle Recherche der Adressen empfohlen."
    )
    story.append(KeepTogether([
        Paragraph("⏱ Schritt 1 — Sofortmassnahme", styles["h2"]),
        Paragraph(urgency_text, styles["body"]),
        Spacer(1, 4),
    ]))

    # ── Schritt 2: Strafanzeige ─────────────────────────────────────────────
    evidence_quality = (
        "Die vorliegende Analyse enthält L1-Beweise (mathematisch bewiesen) "
        "und ist damit gerichtsverwertbar ohne weitere Expertise."
        if has_l1 else
        "Die vorliegende Analyse enthält L2-Indizien. Für ein Strafverfahren "
        "empfiehlt sich die Hinzuziehung eines zertifizierten Blockchain-Forensikers."
    )
    story.append(KeepTogether([
        Paragraph("📋 Schritt 2 — Strafanzeige", styles["h2"]),
        Paragraph(
            "Reichen Sie diesen Bericht als Anlage zur Strafanzeige ein. "
            "Relevante Stellen nach Jurisdiktion:<br/>"
            "<b>Deutschland:</b> Landeskriminalamt (LKA) Cybercrime, Bundeskriminalamt (BKA) "
            "oder zentrale Ansprechstelle Cybercrime (ZAC) des zuständigen LKA.<br/>"
            "<b>Österreich:</b> Bundeskriminalamt — Cybercrime Competence Center (C4).<br/>"
            "<b>Schweiz:</b> Koordinationsstelle zur Bekämpfung der Internetkriminalität (KOBIK).<br/>"
            f"{evidence_quality}",
            styles["body"]
        ),
        Spacer(1, 4),
    ...
    # ── Schritt 3: Exchange kontaktieren (nur wenn Treffer) ──────────────────
    if has_exchange:
        # Kontaktdaten aus DB anreichern
        exchange_detail_lines = []
        for exc_name in exchange_names:
            exc_info = _exchange_db.get(exc_name)
            if exc_info:
                portal_str = f"Portal: {exc_info.le_portal_url}" if exc_info.le_portal_url \
                    else f"E-Mail: {exc_info.le_email}"
                sla = f"{exc_info.response_days_typical}–{exc_info.response_days_max} Werktage"
                direct = "Opfer-Direktkontakt möglich" if exc_info.accepts_victim_direct \
                    else "Nur über Behörden (LE-Request erforderlich)"
                exchange_detail_lines.append(
                    f"<b>{exc_name}</b>: {portal_str} | Antwortzeit: {sla} | {direct}"
                )
            else:
                exchange_detail_lines.append(
                    f"<b>{exc_name}</b>: Kontaktdaten nicht in DB — Exchange-Website prüfen"
                )

        exchange_list_html = "<br/>".join(exchange_detail_lines)
        freeze_detail = (
            f"Identifizierte Exchange(s):<br/>{exchange_list_html}<br/><br/>"
            "Benötigt für alle Freeze-Requests: offizielle Strafanzeige-Bestätigung "
            "(Aktenzeichen). Referenzieren Sie in der Anfrage exakt die Adressen "
            "und TXIDs aus Abschnitt 3 dieses Reports."
        )
        story.append(KeepTogether([
            Paragraph("🏦 Schritt 3 — Exchange-Freeze-Request", styles["h2"]),
            Paragraph(freeze_detail, styles["body"]),
            Spacer(1, 4),
        ]))

    # ── Schritt 4: OFAC-Sanktionen ──────────────────────────────────────────
    if has_sanctioned:
        story.append(KeepTogether([
            Paragraph("⚠ Schritt 4 — OFAC-Sanktions-Meldung", styles["alert"]),
            Paragraph(
                "Mindestens eine Adresse in dieser Analyse steht auf der OFAC-Sanktionsliste. "
                "Jede Transaktion mit dieser Adresse kann US-Sanktionsrecht verletzen. "
                "Melden Sie den Sachverhalt zusätzlich dem zuständigen nationalen "
                "Finanzgeheimdienst (FIU) sowie der Europäischen Bankenaufsicht (EBA).",
                styles["body"]
            ),
            Spacer(1, 4),
        ]))

    # ── Schritt 5 (oder 4): Beweissicherung ─────────────────────────────────
    step_num = 5 if (has_exchange and has_sanctioned) else (4 if has_exchange or has_sanctioned else 3)
    story.append(KeepTogether([
        Paragraph(f"📁 Schritt {step_num} — Beweissicherung", styles["h2"]),
        Paragraph(
            "Sichern Sie alle folgenden Dokumente in unveränderlicher Form:<br/>"
            "• Diesen Forensik-Report (PDF) mit Report-Hash (Abschnitt 5)<br/>"
            "• Alle originalen Transaktionsnachweise und Screenshots<br/>"
            "• Korrespondenz mit Exchange und Behörden<br/>"
            "• Aktenzeichen der Strafanzeige<br/>"
            "Die Transaktions-IDs dieses Reports sind permanent auf der Bitcoin-Blockchain "
            "gespeichert und können jederzeit auf blockstream.info verifiziert werden — "
            "sie sind unveränderlich und unabhängig von diesem Dokument.",
            styles["body"]
        ),
        Spacer(1, 4),
    ]))
    ...
    disclaimer_box = Table([[Paragraph(
        "<b>Rechtlicher Hinweis:</b> Dieser Bericht wurde automatisiert erstellt und stellt "
        "keine Rechtsberatung dar. Konfidenz-Level L1 und L2 sind forensische Einschätzungen "
        "des Systems — sie ersetzen keine juristische Beurteilung durch einen Rechtsanwalt "
        "oder zertifizierten Blockchain-Forensiker. L3- und L4-Befunde sind aus dem "
        "offiziellen Report ausgeschlossen, da sie nicht beweiskräftig sind. "
        "Die Betreiber dieses Systems übernehmen keine Haftung für Entscheidungen "
        "die auf Basis dieses Reports getroffen werden.",
        ParagraphStyle("disc", fontSize=7.5, leading=11,
                       textColor=COLOR_DARK, fontName="Helvetica",
                       parent=styles["body"])
...
def _section_peeling_chain(peeling_result, styles: dict) -> list:
    """Optional section — only rendered if peeling chain was detected."""
    story = [Paragraph("Forensischer Hinweis: Peeling-Chain-Muster", styles["h1"])]
    story += _hr(styles)

    caveat_data = [[Paragraph(
        "\u26a0  L3 \u2014 INDIKATIVER BEFUND: Dieses Muster ist ein forensischer Hinweis, "
        "kein mathematischer Beweis. Nicht als alleinige Grundlage f\u00fcr "
        "rechtliche Massnahmen verwenden.",
        ParagraphStyle("caveat_box", fontSize=8.5, fontName="Helvetica-Bold",
                       textColor=COLOR_WARNING, leading=13)
    )]]
    caveat_tbl = Table(caveat_data, colWidths=[PAGE_W - 2*MARGIN])
    caveat_tbl.setStyle(TableStyle([
        ("BACKGROUND",    (0,0), (-1,-1), colors.HexColor("#FEF9E7")),
        ("LINEABOVE",     (0,0), (-1,-1), 2, COLOR_WARNING),
        ("TOPPADDING",    (0,0), (-1,-1), 8),
        ("BOTTOMPADDING", (0,0), (-1,-1), 8),
        ("LEFTPADDING",   (0,0), (-1,-1), 10),
    ]))
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph(peeling_result.report_summary, styles["body"]))
    story.append(Spacer(1, 6))

    signal_rows = [["Signal", "Status", "Bedeutung"]]
    signal_info = {
        "STRUCTURAL":   ("1-Input / 2-Output Struktur", "Jede TX hat genau 1 Input und 2 Outputs"),
        "AMOUNT_DECAY": ("Konsistente Betragsabnahme",  "\u22640.5% Abnahme pro Hop"),
        "LINEAR":       ("Lineare Kette",               "Kein Branching, ein dominanter Output"),
        "TEMPORAL":     ("Zeitliche Regelm\u00e4\u00dfigkeit", "\u00b130% Intervall-Varianz"),
    }
    active = {s.value for s in peeling_result.signals_summary}
    for key, (label, desc) in signal_info.items():
        status = "\u2713 Aktiv" if key in active else "\u2013 Nicht aktiv"
        col = COLOR_SUCCESS if key in active else COLOR_MID_GREY
        signal_rows.append([
            Paragraph(label, ParagraphStyle("sl", fontSize=8, fontName="Helvetica-Bold",
                                            textColor=col, leading=11)),
            Paragraph(status, ParagraphStyle("ss", fontSize=8, fontName="Helvetica-Bold",
                                             textColor=col, leading=11)),
            Paragraph(desc, styles["body"]),
        ])
    sig_tbl = Table(signal_rows, colWidths=[45*mm, 28*mm, PAGE_W - 2*MARGIN - 73*mm])
    ...
    if peeling_result.peeling_hops:
        hop_rows = [["Hop", "TX (gek\u00fcrzt)", "Betrag (BTC)", "Abnahme", "Zeitversatz"]]
        for h in peeling_result.peeling_hops:
            hop_rows.append([
                str(h.hop_index),
                Paragraph(h.txid[:20] + "...", styles["mono_small"]),
                str(h.amount_btc),
                f"{float(h.decay_pct * 100):.4f}%",
                _fmt_delta(h.time_delta_seconds) if h.time_delta_seconds else "\u2014",
            ])
        hop_tbl = Table(hop_rows, colWidths=[12*mm, 55*mm, 30*mm, 22*mm,
                                              PAGE_W - 2*MARGIN - 119*mm])
        hop_tbl.setStyle(TableStyle([
...
def _section_serial_actor(serial_matches: list, styles: dict) -> list:
    """Serientäter-Warnung — nur wenn Treffer vorhanden."""
    if not serial_matches:
        return []

    story = [Paragraph("⚠ Serientäter-Hinweis", styles["h1"])]
    story += _hr(styles)

    # Alle betroffenen Cases zusammenfassen
    all_cases = sorted({c for m in serial_matches for c in m.matching_cases})
    total_btc  = sum(m.total_btc_involved for m in serial_matches if m.total_btc_involved)

    # Warn-Box
    warn_box = Table([[Paragraph(
        f"<b>WARNUNG:</b> Diese Untersuchung weist {len(serial_matches)} Überschneidung(en) "
        f"mit {len(all_cases)} anderen bekannten Fall/Fällen auf. "
        f"Dies deutet auf einen Serientäter hin. "
        f"Betrag über alle verbundenen Fälle: ca. {total_btc:,.4f} BTC. "
        f"Verbundene Fälle: {', '.join(all_cases)}",
        ParagraphStyle("wb", fontSize=9, leading=13, fontName="Helvetica-Bold",
                       textColor=COLOR_ALERT)
    )]], colWidths=[PAGE_W - 2*MARGIN])
    warn_box.setStyle(TableStyle([
        ("BACKGROUND",    (0, 0), (-1, -1), colors.HexColor("#FDEDEC")),
        ("BOX",           (0, 0), (-1, -1), 1.2, COLOR_ALERT),
    ...
    # Treffer-Tabelle
    rows = [["Typ", "Gemeinsamer Wert", "Betroffene Fälle", "Konfidenz", "BTC gesamt"]]
    for m in serial_matches:
        rows.append([
            m.match_type.replace("_", " "),
            Paragraph(m.shared_value[:32] + ("..." if len(m.shared_value) > 32 else ""),
                      ParagraphStyle("ms", fontSize=7, fontName="Courier",
                                     textColor=COLOR_ACCENT, wordWrap="CJK")),
            ", ".join(m.matching_cases[:3]) + ("…" if len(m.matching_cases) > 3 else ""),
            m.confidence_level,
            f"{m.total_btc_involved:,.4f}" if m.total_btc_involved else "—",
        ])

    tbl = Table(rows, colWidths=[32*mm, 55*mm, 42*mm, 18*mm, PAGE_W - 2*MARGIN - 147*mm])
    tbl.setStyle(TableStyle([
        ("BACKGROUND",    (0, 0), (-1, 0), COLOR_ALERT),
        ("TEXTCOLOR",     (0, 0), (-1, 0), COLOR_WHITE),
        ("FONTNAME",      (0, 0), (-1, 0), "Helvetica-Bold"),
    ...
    story.append(Spacer(1, 6))
    story.append(Paragraph(
        "Empfehlung: Reichen Sie alle verbundenen Fälle gemeinsam bei den Behörden ein. "
        "Ein dokumentierter Serientäter erhöht die Ermittlungspriorität erheblich und "
        "verbessert die Chancen auf ein Freeze-Request bei der Exchange.",
        styles["body"]
    ))
    return story
...
def generate_report(
    ...
    Args:
        ...
        peeling_result: Optional PeelingChainResult
        victim_name:    Name of the victim (shown on cover page)
        victim_contact: Email/phone of victim (shown on cover page)
        language:       'de' (default) or 'en' (DE text used throughout, EN planned)
        serial_matches: Optional list of SerialMatch objects
    ...
    if peeling_result and peeling_result.detected:
