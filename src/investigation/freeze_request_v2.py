...
# ---------------------------------------------------------------------------
# Exchange contact lookup — delegiert an exchange_contacts.py
# ---------------------------------------------------------------------------

_contact_db = _ExchangeContactDB()


@dataclass
class ExchangeContact:
    """Lokaler Adapter — wraps ExchangeContactDB für FreezeRequest-Kompatibilität."""
    name: str
    compliance_email: str
    le_portal: Optional[str]
...
def get_exchange_contact(exchange_name: str) -> ExchangeContact:
    """Gibt Kontaktdaten für eine Exchange zurück — aus der zentralen DB."""
    exc = _contact_db.get(exchange_name)
    if exc:
        return ExchangeContact(
            name=exc.name,
            compliance_email=exc.le_email or f"compliance@{exc.name.lower()}.com",
            le_portal=exc.le_portal_url,
            freeze_sla_hours=exc.response_days_typical * 24,
            notes=exc.process_notes_en,
        )
    # Fallback für unbekannte Exchanges
    return ExchangeContact(
        name=exchange_name,
        compliance_email=f"compliance@{exchange_name.lower().replace(' ', '')}.com",
        le_portal=None,
        freeze_sla_hours=72,
        notes="Contact details not in database. Verify via exchange website.",
    )
